...
public class MissingAndRepeatedNumberInArray {
	public static void main(String[] args) {
		...
		Integer[] arr = {3,1,2,5,5};
		// expected sum 15, expected sum sq 55
		// actual sum 16 actual sum sq 64 
		List<Integer> A =  Arrays.asList(arr);
		ArrayList<Integer> output = repeatedNumber(A);
		for (Integer integer : output) {
			System.out.println(integer);
		}
	}
	// Sum Expected  = Sum Actual  + Duplicate - Missing
	// Sum Expected Squares = Sum Actual Squares + Duplicate Sq - Missing Sq
	
	// Sum Expected - Sum Actual = Duplicate - Missing
	// Sum Expected Squares - Sum Actual Squares = Duplicate Sq - Missing Sq
	
	// Sum of n natural numbers = n*(n+1)/2
	// Sum Squares of n natural numbers = n*(n+1)*(2*n+1)/6  
    public static ArrayList<Integer> repeatedNumber(final List<Integer> A) {
    	ArrayList<Integer> output = new ArrayList<>();
    	Integer n = A.size();
    	Integer expectedSum = n*(n+1)/2;
    	Integer expectedSumSquares = n*(n+1)*(2*n+1)/6;
    	Integer actualSum=0;
    	Integer actualSumSquares=0;
    	for (int i = 0; i < n; i++) {
			actualSum+=A.get(i);
			actualSumSquares+=A.get(i)*A.get(i);
		}
